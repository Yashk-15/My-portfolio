...
function TechChip({ tech }) {
  ...
  return (
    <span style={{
      ...
      <span style={{ color: def?.color ?? 'var(--muted)', display: 'flex' }}>{def?.icon}</span>
      {tech}
    </span>
  );
}
...
// Uses plain <div> with CSS transitions instead of motion.div to avoid
...
function ProjectCard({ proj, i }) {
  ...
  return (
    <div
      ...
      onTouchStart={e => {
        e.currentTarget.classList.add('project-card--touch');
      }}
      onTouchEnd={e => {
        setTimeout(() => e.currentTarget.classList.remove('project-card--touch'), 300);
      }}
    >
      {/* ── Screenshot preview ── */}
      {proj.category === 'app' ? (
      ...
      <div style={{ padding: '18px 20px 20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div style={{ flex: 1 }}>
            <h3 style={{ color: 'var(--text)', fontWeight: 700, fontSize: '1rem', marginBottom: 2 }}>
              {proj.title}
            </h3>
            <p style={{ color: 'var(--accent)', fontSize: '0.71rem', fontWeight: 600 }}>{proj.subtitle}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 8, flexShrink: 0 }}>
            <span style={{ color: 'var(--subtle)', fontSize: '0.68rem' }}>{proj.date}</span>
        ...
        <ul style={{ display: 'flex', flexDirection: 'column', gap: 5, flex: 1, marginBottom: 16 }}>
          {proj.highlights.map((h, idx) => (
            <li key={idx} style={{ display: 'flex', gap: 7, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.5 }}>
    ...
    <section id="projects" style={{ padding: '100px 0', borderTop: '1px solid var(--border)' }}>
      <div className="container-main">
        ...
        >
          ...
          <h2 style={{ color: 'var(--text)', fontSize: '1.8rem', fontWeight: 700, letterSpacing: '-0.02em' }}>
            Projects
          </h2>
        </motion.div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 40 }}>
          {categories.map(cat => {
            ...
            return (
              ...
              >
                {cat.icon} {cat.label}
                <span className={`cat-count${isActive ? ' cat-count--active' : ''}`}>{count}</span>
              </button>
            );
          })}
        </div>

        {/* Cards — AnimatePresence only for tab switch, not per-card */}
      ...
      <style>{`
        ...
        .cat-tab--active {
          border-color: rgba(61,90,128,0.3) !important;
          background: var(--accent-dim) !important;
